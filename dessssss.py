...
def encryptor(path):
	...
	#Encrypting using single DES	
	print("Wait it is being encrypting.....\n")	
	try:
		
		cipher=DES.new(key_enc[:8],DES.MODE_CBC,key_enc[24:32])
		ciphertext=cipher.encrypt(image)
	...
	ciphertext+=hash_of_original.digest()
...
def decryptor(encrypted_image_path):
	...
	#decrypting using single DES
	print("	Decrypting...")
	try:
		
		cipher=DES.new(key_dec[:8],DES.MODE_CBC,key_dec[24:32])
	...
	hash_of_decrypted=SHA256.new(data=plaintext)
